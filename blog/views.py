...
def postpreference(request, postid, userpreference):
    # print(request.method, postid, userpreference)
    if request.method == "POST":
        eachpost = get_object_or_404(Post, id=postid)
        obj = ''
        valueobj = ''
        try:
            obj = Preference.objects.get(user=request.user, post=eachpost) # 좋아요 관계가 이미 있는 경우
            valueobj = obj.value # 1 or 2
            valueobj = int(valueobj)
            userpreference = int(userpreference) # 1이면 좋아요, 2이면 싫어요
        ...
        except Preference.DoesNotExist: # 좋아요 관계가 없을 경우
            upref = Preference()
            upref.user = request.user
            upref.post = eachpost
            upref.value = userpreference
